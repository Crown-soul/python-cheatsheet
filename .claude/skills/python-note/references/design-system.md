...
8. **Footer**
9. **`</body>` 前一行**：`<script src="annotate.js?v=N"></script>`——選字做筆記的功能，三份筆記共用同一個檔案。新筆記照加，小抄類不加。**`?v=N` 的版號要記得帶**：改了 `annotate.js` 的內容，就把三份筆記裡這個版號一起 +1，不然使用者瀏覽器裡快取的舊版 `annotate.js` 可能撐好一陣子才會換新——版號不同，瀏覽器才會當成不同資源重抓。
